        ...
        <div className="grid gap-0" style={{ gridTemplateColumns: `repeat(${map[0].length}, minmax(0, 1fr))` }}>
          {map.map((row, y) =>
            row.map((tile, x) => (
              <div
                key={`${x}-${y}`}
                className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center text-lg md:text-2xl border border-black/10 hover:bg-white/30 transition-colors cursor-pointer"
                onClick={() => {
